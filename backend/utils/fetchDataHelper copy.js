...
for (const field of fieldArray) {
  ...
  // If the field has a custom nested path, push it first
  console.log("Processing field:", trimmed, customFields);
  if (customFields[trimmed]) {
    // If it has a dot in it push it directly
    if (customFields[trimmed].includes('.')) {
      orArray.push({ [customFields[trimmed]]: { $regex: search_term, $options: "i" } });
      continue; // move to next field
    }
    const nestedField = `${customFields[trimmed]}.${trimmed}`;
    orArray.push({ [nestedField]: { $regex: search_term, $options: "i" } });
  }

  // Always push the top-level fallback
  orArray.push({ [trimmed]: { $regex: search_term, $options: "i" } });
}
...
// const queryBuilder = (payload = {}, options = {}, Model) => {
//   const {
//     page = 1,
//     fields = [],
//     search_term = "",
...
//   if (fields.length && search_term && Model) {
//     const fieldArray = Array.isArray(fields) ? fields : String(fields).split(",");
//     const orArray = [];

//     for (const field of fieldArray) {
//       const trimmed = field.trim();
//       const fieldPath = customFields[trimmed] ? `${customFields[trimmed]}.${trimmed}` : trimmed;

//       // ✅ Only use schema path if Model is provided
//       const schemaType = Model.schema.path(trimmed) || Model.schema.path(customFields[trimmed]);
//       if (schemaType && schemaType.instance === "String") {
//         orArray.push({ [fieldPath]: { $regex: search_term, $options: "i" } });
//       }
//     }

//     if (orArray.length) query.$or = orArray;
//   }
...
const fetchData = async (payload, Model, options = {}) => {
  ...
  const hasNestedSearch =
    ...
    Object.keys(options.custom_fields).some((key) => {
      ...
      return query.$or?.some?.((cond) =>
        Object.keys(cond)[0].startsWith(path + ".")
      );
    });
  ...
  const hasSharedIdPattern = options.custom_fields &&
    Object.values(options.custom_fields).some(fieldDef => {
      const path = typeof fieldDef === 'string' ? fieldDef : (fieldDef.path || '');
      return path.includes('._id') && path.includes('.');
    });

  const runPipeline = hasNestedSearch || needsRefFilter || hasSharedIdPattern;
  ...
  if (runPipeline) {
    ...
    console.log("Running standard find query", query);
