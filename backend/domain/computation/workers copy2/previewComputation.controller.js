...
async function processPreviewStudentResults(
  ...
  // ✅ CRITICAL CHECK: Only calculate outstanding courses if NOT in termination/withdrawal
  let outstandingCourses = [];

  // Check if student is NOT in termination or withdrawal status
  const isTerminatedOrWithdrawn =
    academicStanding.remark?.toUpperCase().includes('TERMINATED') ||
    academicStanding.remark?.toUpperCase().includes('WITHDRAW') ||
    academicStanding.status?.toLowerCase().includes('terminated') ||
    academicStanding.status?.toLowerCase().includes('withdraw') ||
    academicStanding.status?.toLowerCase().includes('withdrawal');

  if (!isTerminatedOrWithdrawn) {
    console.log(academicStanding, "Helloworld")
    // Only calculate outstanding courses for active students
    outstandingCourses = await GPACalculator.calculateOutstandingCourses(
      student._id,
      activeSemester._id
    );
  } else {
    // Log for debugging purposes in preview
    console.log(`📋 [PREVIEW] Skipping outstanding courses for ${student.matricNumber} - Status: ${academicStanding.remark}`);
  }
  ...
  // ✅ Also log outstanding courses status
  console.log(`  Outstanding Courses: ${outstandingCourses.length} (Terminated/Withdrawn: ${isTerminatedOrWithdrawn})`);
  ...
  return {
    ...
    level: studentLevel,
    isPreview: true,
    isTerminatedOrWithdrawn: isTerminatedOrWithdrawn, // Add this for tracking
...
async function finalizePreviewComputation(
  ...
  // Fetch department leadership details
  ...
  computationSummary.departmentDetails = departmentDetails;
